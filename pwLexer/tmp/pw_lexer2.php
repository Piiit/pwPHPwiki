...
class pwLexer {
	...
	*/
	...
	protected $dbginf = array();
	protected $parsed = false;
	protected $norecursion = array();
	public static $version = "0.42d - PIWO";
	public $AST = array();  // AST: Abstract Syntax Tree

	public function setNoRecursion($for) {
		if (is_array($for)) {
			foreach ($for as $nodeName) {
				$this->addNoRecursion($nodeName);
			}
		} else {
			$this->addNoRecursion($for);
		}
	}

	protected function addNoRecursion($nodeName) {
		if (!is_string($nodeName)) {
			throw new InvalidArgumentException("Node names must be either string or an array of strings!");
		}
		$this->norecursion[] = $nodeName;
	}
	...
	public function __construct($fulltxt, $debug = false) {
		...
		$this->documentToken = new pwPattern("#DOCUMENT", "TYPE_BASE");
		$this->textToken = new pwPattern("#TEXT", "TYPE_BASE");
		$this->patterntable = array("#DOCUMENT" => $this->documentToken, "#TEXT" => $this->textToken);
	...
	protected function getTokenName($m) {
		...
		#pw_debug($this->ids);
		$this->addLog("FATAL", "ID '$id' not found in patternorder-table!");
		return false;
	...
	protected function getToken(&$txt) {
		...
		$name = $this->AST[$pid]["NAME"];
		//$type = $this->getPatternInfo($name, "TYPE");
		$pattern = $this->getPattern($name);
		//$type = $pattern->getType();

		if ($pattern->getType() == "TYPE_ABSTRACT") {
			$pid = $this->AST[$pid]["PARENT"];
		}

		//$mode = $this->getPatternInfo($this->AST[$pid]["NAME"], "NAME");
		$parentPattern = $this->getpattern($this->AST[$pid]["NAME"]);
		$mode = $parentPattern->getName();
		...
		if (! preg_match($regex, $txt, $m, PREG_OFFSET_CAPTURE)) {
			...
			$expected = stripslashes($parentPattern->getExitPattern());
	...
	protected function updatePatternOrder($pattern, $name, $isexit = false) {
		...
		$this->patternorder += $this->patterntable[$origname]->getLevel();
	}
	...
	public function connectTo($name, $to) {
		...
		*/
		if ($this->patterntable[$name]->getType() == "TYPE_ABSTRACT") {
			$this->addLog("WARNING", "CONNECTTO: Can't connect two ABSTRACT Nodes! '$name->$to' failed!");
			return false;
		}

		if ($this->patterntable[$name]->getConnectTo() != false) {
			$this->addLog("INFO", "CONNECTTO for '$name->".$this->patterntable[$name]->getConnectTo()."' already set in patterntable! Will be altered to '$name->$to'!");
		}

		$this->patterntable[$name]->setConnectTo($to);
		$this->patterntable[$to] = new pwPattern($to, "TYPE_ABSTRACT");
		$this->addLog("INFO", "CONNECTTO: '$name->$to' connected.");
		return true;
	}
	...
	public function addPattern($pattern) {
		if (get_class($pattern) == "pwPattern") {
			$this->patterntable[$pattern->getName()] = $pattern;
		} else {
			throw new InvalidArgumentException("Pattern must be of class type 'pwPattern'!");
		}
	}
	...
	public function addLinePattern($name, $entrypattern, $exitpattern='\n', $flags = 0) {
		...
		if (strlen($entrypattern) == 0) {
			$this->addLog("WARNING", "Patternstringlength == 0! Pattern not added. name='$name', entry='$entrypattern', exit='$exitpattern', type='LINE'");
			return false;
		}
	...
	private function prepareAllowedModeList() {
		foreach ($this->patterntable as $token) {
			foreach ($token->getAllowedModes() as $mode) {
				if (! array_key_exists($mode, $this->patterntable)) {
					throw new UnexpectedValueException("Tried to prepare allowed modes for '".$token->getName()."', but mode '$mode' does not exist in patterntable!");
				}
				$this->patterntable[$mode]->addMode($token->getName());
			}
		}
	}
	...
	public function setAllowedModes($name, $modes, $initself = false) {
		...
		//TODO move to pw_token somehow...
		if ($this->patterntable[$mode]->getAllowedModes() == null) {
		$this->patterntable[$mode]->setAllowedModes(array());
		}
		*/
		/*
	...
	*/
	...
	public function addSectionPattern($name, $entrypattern, $exitpattern, $flags = 0) {
		...
		if (strlen($entrypattern) == 0 or strlen($exitpattern) == 0) {
			$this->addLog("WARNING", "Patternstringlength == 0! Pattern not added. name='$name', entry='$entrypattern', exit='$exitpattern', type='SECTION'");
			return false;
		}
	...
	public function getPatternString($statusname = "#DOCUMENT", $keysensitive = false) {
		...
		$pattern = $this->getPattern($statusname);
		...
		*/
		if ($pattern->getCachedRegexpRenew() === false) {
			$this->ids = $pattern->getModes();
			return $pattern->getCachedRegexp();;
		}
		...
		$patternString .= $exitpattern;
		} else {
		$patternString = rtrim($pattern, '|');
		}
		*/
		if (count($pattern->getModes()) > 0) {
			foreach($pattern->getModes() as $mode) {
				$modePattern = $this->getPattern($mode);
				$entrypattern = $modePattern->getEntryPattern();
				$newmodeorder[$this->patternorder] = $mode;
				$this->updatePatternOrder($entrypattern, $mode);
				$patternString .= $entrypattern.'|';
			}
		...
		$pattern->setModes($newmodeorder);

		$exitpattern = $pattern->getExitPattern();
		/*if (isset()) {
		 $exitpattern = $pattern->getExitPattern();
		}*/
		if ($exitpattern !== NULL) {
			$pattern->addChildMode("__exit__$statusname", $this->patternorder);
			$this->updatePatternOrder($exitpattern, $statusname, true);
			$patternString .= $exitpattern;
		} else {
			$patternString = rtrim($patternString, '|');
		}

		$patternString = $regex_start.$patternString.$regex_end.$regex_param;

		//$this->patterntable[$statusname]["CACHED_REGEXP"] = $pattern;
		//$this->patterntable[$statusname]["CACHED_REGEXP_RENEW"] = false;
		$pattern->setCachedRegexp($patternString);
		$pattern->setCachedRegexpRenew(false);
		...
		*/
		if (count($pattern->getModes()) == 0) {
			throw new Exception("No Modes for '$statusname' given! Can't determine context...");
		}
	...
	private function getPattern($name) {
		...
		if (!is_string($name)) {
			//$this->addLog("FATAL", "'\$name has to be string. ".strtoupper(gettype($statusname))." given!");
			throw new InvalidArgumentException("Name '$name' has to be string! ".gettype($name)." given!");
			//return false;
		}
		 
		$this->stripExitTagName($name);
		if (!array_key_exists($name, $this->patterntable)) {
			//$this->addLog("WARNING", "Pattern with name '$name' doesn't exist!");
			//return false;
			throw new Exception("Name '$name' not found in patterntable!");
		}
		return $this->patterntable[$name];
	}
	...
	protected function stripExitTagName(&$name) {
		...
		if (! is_string($name)) {
			...
			throw new InvalidArgumentException("Wrong datatype. \$name must be string!");
			return NULL;
		}
	...
	public function printDebugInfo($linesbefore = -1, $linesafter = -1, $print = true) {
		...
		if (is_array($di)) {
			$o = "<table class='lexertable stripes' style='color: black; background-color: lightgray'>".
			...
			foreach ($di as $d) {
				...
				$o .=  "<tr>".
						...
						"</pre></td>".
						"<td>".$d["ABSTRACTNODE"].$d["NAME"]."<br>".$this->getPatternInfo( $d["NAME"], "TYPE" )."</td>".
	...
	public function getArray($node, $arr = array()) {
		...
		for ($node = $this->firstChild($node); $node != null; $node = $this->nextSibling($node)) {
			...
			if ($node['NAME'] == "#TEXT") {
				...
				if ($this->hasChildNodes($node) and !in_array($node['NAME'], $this->norecursion)) {
